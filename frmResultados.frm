...
Static iRetrasoResultados As Integer
    ...
    If G_PUBLICAR_POSICION Then
        'Mantenemos publicadas las últimas fases de cada grupo( ya que al publicar se borran las anteriores)
    ...
    Else
        ' Solo imprimimos los dorsales de las categorías que no han salido a bailar
        ' y por lo tanto no tienen puntuaciones O los que se publican parcialmente
        ' más los X siguientes grupos del horario de la hora actual
        ' que no tengan puntuaciones y que tengan dorsales y que no estén publicados por duplicado
    ...
    If VarCfg("publicidad_activa") = "N" And Not rs.EOF Then
        ...
        Do While Not rs.EOF
            If dHoraAntPublicacion <= rs!hora_publicacion Or rs!hora_publicacion = dHoraUltimaPublicacion Then
                ...
                If rs.EOF Then
                    rs.MoveFirst
                    'Evitamos publicar los resultados que están en el panel derecho
...
Dim iCarPorLinea As Integer
    ...
    If bTeamMatch Then
        ...
        If pbCuadro.Tag = "Libreta" Then
            ...
            While Not rs.EOF
                ...
                End If
                PrintConSombra pbCuadro, sLado & i & "º - " & rs!comunidad & " con " & rs!puntuacion & " Ptos."
                rs.MoveNext
            Wend
            rs.Close
        Else
            ...
            While Not rs.EOF
                If dPuntos <> rs!puntuacion Then
                    ...
                    dPuntos = rs!puntuacion
                End If
                PrintConSombra pbCuadro, sLado & i & "º - " & rs!comunidad & " -> " & rs!puntuacion & " Ptos."
                rs.MoveNext
            Wend
            rs.Close
            pbCuadro.Print
    ...
    Dim lbOrden As ListBox
                    ...
                    If iFase = 1 Then ' FINAL
                        Set rsPuestos = db.OpenRecordset("SELECT puesto FROM cal_conjunto WHERE cod_categoria = " & iCategoria & " AND num_dorsal = " & rsParejas!num_dorsal & " AND regla='FIN' ORDER BY 1", dbOpenSnapshot)
                            If rsPuestos.EOF Then
                                ' Si todavía no hay cálculos se muestra No Calculado NC
                                sCad = sCad & "(--)"
                            Else
                                sCad = sCad & "(" & Format$(Val(rsPuestos!puesto), "#") & "º)"
                            End If
                        rsPuestos.Close
                    Else ' No final
        ...
        For iCParejas = iInicial To iFinal Step iSalto
            ...
            ' Para que quepan el máximo de parejas en una columna hay que ajustarlas
