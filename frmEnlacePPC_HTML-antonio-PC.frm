...
Begin VB.Form frmEnlacePPC_HTML 
   ...
   ClientTop       =   345
   ClientWidth     =   10695
   Icon            =   "frmEnlacePPC_HTML.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.Frame frmPPC 
      ...
      Begin VB.CommandButton cmdCategAct 
         Height          =   375
         Left            =   9060
         Picture         =   "frmEnlacePPC_HTML.frx":0442
         Style           =   1  'Graphical
         TabIndex        =   73
      ...
      Begin VB.CommandButton cmdDorsales 
         Height          =   375
         Left            =   9840
         Picture         =   "frmEnlacePPC_HTML.frx":074C
         Style           =   1  'Graphical
         TabIndex        =   72
      ...
      Begin VB.CommandButton cmdDescalif 
         Height          =   435
         Left            =   10020
         Picture         =   "frmEnlacePPC_HTML.frx":126E
         Style           =   1  'Graphical
         TabIndex        =   71
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VB.CommandButton cmdSubirDatos 
            ...
            Height          =   360
            Left            =   90
            Picture         =   "frmEnlacePPC_HTML.frx":1E50
            Style           =   1  'Graphical
            TabIndex        =   29
            Top             =   225
            Width           =   405
      ...
      Begin VB.Frame Frame3 
         ...
         Begin VB.CommandButton cmdSubirDatosSigFase 
            ...
            Height          =   360
            Left            =   90
            Picture         =   "frmEnlacePPC_HTML.frx":22BA
            Style           =   1  'Graphical
            TabIndex        =   21
            Top             =   270
            Width           =   405
      ...
      Begin VB.ComboBox cbPista 
         ...
         EndProperty
         Height          =   390
         ItemData        =   "frmEnlacePPC_HTML.frx":2724
         Left            =   765
         List            =   "frmEnlacePPC_HTML.frx":2746
      ...
      Begin VB.CommandButton cmdDatos 
         ...
         Height          =   435
         Left            =   9060
         Picture         =   "frmEnlacePPC_HTML.frx":2779
         Style           =   1  'Graphical
         TabIndex        =   16
      ...
      Begin VB.CommandButton cmdSelComp 
         ...
         Height          =   360
         Left            =   1845
         Picture         =   "frmEnlacePPC_HTML.frx":2A5B
         Style           =   1  'Graphical
         TabIndex        =   14
         Top             =   360
         Visible         =   0   'False
      ...
      Begin VB.CommandButton cmdSelCat 
         ...
         Height          =   360
         Left            =   1845
         Picture         =   "frmEnlacePPC_HTML.frx":2EC5
         Style           =   1  'Graphical
         TabIndex        =   13
         Top             =   855
         Width           =   450
      ...
      Begin VB.CommandButton cmdSelFase 
         ...
         Height          =   360
         Left            =   1845
         Picture         =   "frmEnlacePPC_HTML.frx":332F
         Style           =   1  'Graphical
         TabIndex        =   12
         Top             =   1305
         Width           =   450
      ...
      Begin VB.CommandButton cmdPuntuaciones 
         Height          =   435
         Left            =   9540
         Picture         =   "frmEnlacePPC_HTML.frx":3799
         Style           =   1  'Graphical
         TabIndex        =   1
...
Private Sub Timer1_Timer()
    ...
    'Comprobamos la información de carga de baterias
    For i = 0 To C_MAX_BATERIA_PANEL - 1
        lblBat(i).Visible = False
        lblCon(i).Visible = False
    Next
    i = 0
    Set rs = db.OpenRecordset("SELECT * FROM bateria_html ORDER BY id_pda", dbOpenSnapshot)
    While Not rs.EOF
        'Comprobamos si este id_pda está asociado a algún juez
        Set rsJuez = db.OpenRecordset("SELECT cod_juez FROM jueces_html WHERE id_pda = " & rs.Fields("id_pda"), dbOpenSnapshot)
        If Not rsJuez.EOF Then
            sIdJuez = rsJuez.Fields("cod_juez")
        Else
            sIdJuez = rs.Fields("id_pda")
        End If
        rsJuez.Close
        
        sHora = rs.Fields("hora_op")
        If Not IsDate(sHora) Then
            lDif = 0
        Else
            lDif = DateDiff("s", Time(), CDate(sHora))
            lDif = lDif Mod 3600
        End If
        
        lblBat(i).Visible = True
        lblBat(i).Caption = "(" & sIdJuez & ")" & Val(rs.Fields("bateria")) & "%" & vbCrLf & lDif & "s"
        lblCon(i).Visible = True
        
        If Abs(lDif) > C_MAX_SEG_CON Then
            lblCon(i).BackColor = vbYellow
        Else
            lblCon(i).BackColor = vbGreen
        End If
        
        'Comprobamos la carga
        If Val(rs.Fields("bateria")) > C_BATERIA_MEDIA Then
            lblBat(i).BackColor = vbGreen
        ElseIf Val(rs.Fields("bateria")) > C_BATERIA_BAJA Then
            lblBat(i).BackColor = vbYellow
        Else
            lblBat(i).BackColor = vbRed
        End If
...
Dim rs As Recordset
    ...
    If G_CALCULO_AUTO_PPC And chkGenSigCat.Value = 1 Then
        ...
        If ComprobarSiEstanTodasPuntuaciones(Val(tbCodCatAct.Text), chkRepAct.Value, tbCodFaseAct.Text) Then
            'Comprobamos si aceptamos los descalificados puestos por los jueces (Solo para fases finales)
            If C_NUM_JUECES_ACEPTAR_NO_PRESENTES > 0 And iFase = 1 Then
                Set rs = db.OpenRecordset("SELECT num_dorsal, COUNT(*) as contador FROM puntuaciones WHERE cod_categoria = " & Val(tbCodCatAct.Text) & _
                                            " AND repesca = " & chkRepAct.Value & " AND fase = " & tbCodFaseAct.Text & " AND puesto =0 GROUP BY num_dorsal", dbOpenSnapshot)
                While Not rs.EOF
                    If rs.Fields("contador") > C_NUM_JUECES_ACEPTAR_NO_PRESENTES Then
                        db.Execute "UPDATE dorsales SET no_presente = 1 WHERE num_dorsal = " & rs.Fields("num_dorsal") & _
                                " AND cod_categoria = " & Val(tbCodCatAct.Text) & " AND fase = " & tbCodFaseAct.Text & " AND repesca = " & chkRepAct.Value
                    End If
                    rs.MoveNext
                Wend
                rs.Close
            End If
            'Están todas las puntuaciones por lo que avanzamos de categoría
            If tbCodCatSig.Text = "" Then
                'No hay siguiente categoría
                MsgBox mml_FRASE0595, vbOKOnly Or vbInformation, mml_FRASE0096
            Else
