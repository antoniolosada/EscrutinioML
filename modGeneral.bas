...
'*********************************
'Versiones
Global Const VERSION_VALENCIA = 1
Global Const VERSION_VIGO = 2
Global Const VERSION_COUNTRY_FEBD = 3
Global Const VERSION_COUNTRY_ASOCIACION = 4

'************************************************************************************************************************************
Public Const C_DEBUG = False
Public Const VERSION = VERSION_VALENCIA
Global Const C_COUNTRY = False
'************************************************************************************************************************************


'*********************************
Global Const C_ERROR_FECHA = False
Global Const C_FECHA_ERROR = "04/05/2099"
'*********************************
Public Const C_PASOS = True
...
Global Const MAX_GRUPO_HORARIO = 32 'Tamaño máximo de nombre de grupo para la impresión del horario
Global Const MAX_GRUPO_HORARIO_COORD = 100 'Tamaño máximo de nombre de grupo para la impresión del horario
...
Public Const G_HOJA_IMPRESA = 1
Public Const G_HOJA_NO_IMPRESA = 0
...
Public Const C_COUNTRY_AMATEUR = "Medallas"
...
'Control de puntuaciones para cálculo automático en pantalla de PDAs
Public Const C_FALTAN_PUNTUACIONES = 0
Public Const C_TODAS_PUNTUACIONES = 1
Public Const C_PUNTUACIONES_CINCO_BAILES = 2
...
Global Const C_MARGEN_IZQ_TABLAA_POR_NOMBRE = 20
...
Public Const G_DURACION_DEFECTO_FASES_SEPARACION = 15

Public Const C_MARGEN_CELDA = 80

Public Const MAX_PARAMETROS_MA = 50
...
Public Const C_COLOR_BLANCO = &HFFFFFF
Public Const C_COLOR_AMARILLO_CLARO = &HC0FFFF
...
Public G_MIN_JUECES_DESCALIFICACION As Integer
Public G_IMPRESORA_DEFECTO As String

Global sExecSQL As String
Global sSQL As String
Global G_COD_MODALIDAD_MEDALLAS As String
...
Global G_CATEG_EQUIV_HORARIO As String
Global G_CATEG_XML_AEBDC As String
Global G_COUNTRY_COMB_GRUP_HOJAS As Boolean
Global G_BLOQUEAR_PAREJAS_AEBDC As Integer
Global G_COUNTRY_MODALID_SIN_GR_EDAD As String
Global G_COUNTRY_IMP_TODOS_BAILES_JUNTOS As Boolean
Global G_IMP_POS_BAILES_DIPLOMAS As Boolean
Global G_MARGEN_EPA_Y As Integer
Global G_RESULTADOS_ALTO_CONTRASTE As Boolean
Global G_DIPLOMAS_PRIMER_DORSAL_SALON As Integer
Global G_FORMATO_PAREJAS_PASOS As String
Global G_INTERVALO_CONTROL_CALCULO_PDA As Long
Global G_CONTROL_PUNTUACIONES_NO_PRESENTES As Boolean
Global G_LIM_JUECES_PARA_TABLAS_FINAL_MA As Integer
Global G_CATEGORIAS_EXPORT_PUNTUACIONES As String
Global C_FICHERO_FEDERACION_V2 As String
Global G_DIR_7ZIP As String
Global G_EDADES_COUNTRY_SIN_CONTROL As String
...
Global G_COUNTRY_MODALID_CALC_GRUPOS_EDAD As String
...
Global Const G_MAX_FILAS_POR_PAG_HOR = 32
...
Public Const C_MAX_PARAMETROS_PAG_MA = 40
...
Dim rs As Recordset
    Set rs = db.OpenRecordset("SELECT valor FROM cfg WHERE variable='" & sVar & "'")
        ...
        Else
            ...
            ElseIf sValor = "NO_ASIGNADO" Then
                VarCfg = sValor
            Else
                VarCfg = sValor
                db.Execute "INSERT INTO cfg VALUES ('" & sVar & "',""" & sValor & """,'" & sDesc & "',""" & sDescIngles & """,""" & sValor & """)"
            End If
        End If
    rs.Close
End Function
Public Sub DibujarTablaA(f As Printer, iPosX As Integer, iPosY As Integer, iFilas As Integer, iCols As Integer, aTabla() As String, aDefCelda() As TCelda, iAltoCelda As Integer, Optional iFilaIni As Integer = 0)
...
Dim sTipoLetra As String
    ...
    For iFila = 0 To iFilas - iFilaIni
    ...
    For iCol = 0 To iCols
        ...
        End If
        f.CurrentY = iPosY
        f.Line -Step(0, (iFilas - iFilaIni) * iAltoCelda)
    Next iCol
    For iFila = 0 To iFilas - 1 - iFilaIni
        For iCol = 0 To iCols - 1
            sValor = aTabla(iFila + iFilaIni, iCol)
            If Left$(sValor, 1) = "." And Len(sValor) >= 4 Then
                iTamFuente = Val(Mid$(sValor, 2, 2))
                sTipoLetra = Mid$(sValor, 4, 1)
                sValor = Mid$(aTabla(iFila + iFilaIni, iCol), 5)
            Else
                sTipoLetra = ""
...
Function BuscarCodGrupoEdad(sGrupoEdad As String) As String
Dim rs As Recordset

    On Local Error GoTo error
    
    If sGrupoEdad <> "" Then
        Set rs = db.OpenRecordset("SELECT codigo FROM gruposedad WHERE nombre = '" & sGrupoEdad & "' OR nombre = '*" & sGrupoEdad & "'", dbOpenSnapshot)
        If rs.EOF Then
            BuscarCodGrupoEdad = ""
            Exit Function
        Else
            BuscarCodGrupoEdad = rs!codigo
        End If
        rs.Close
    End If
    Exit Function
...
Public Function DividirCampo(ByVal sCampo As String, aeMail() As String, sSeparador As String, Optional sBorrarCad As String = "") As Integer
Dim j As Integer

    DividirCampo = 1
    If sBorrarCad <> "" Then
        sCampo = Replace(sCampo, sBorrarCad, "")
    End If
...
Dim aCateg(30) As String
Dim iNumEquiv As Integer
    ...
    Select Case UCase(sCat)
        ...
        Case Else
            sEquivCateg = "A"
            'En caso de que no se encuentre la categoría utilizamos la configuración de equivalencias
            iNumEquiv = DividirCampo(G_CATEG_EQUIV_HORARIO, aCateg, ":")
            Dim i As Integer
            For i = 0 To iNumEquiv - 1
                If InStr(Mid(aCateg(i), 2), """" & sCat & """") > 0 Then
                    sEquivCateg = Left(aCateg(i), 1)
                End If
            Next
    End Select
End Function
...
End Function
Function ComprobarSiEstanTodasPuntuacionesDorsalesPresentes(iCodCat As Long, iHojaRepesca As Integer, iFase As Integer) As Integer
Dim iPuestos As Integer
Dim iPuestosDorsalesNoPresentes As Integer
Dim iJueces As Integer
Dim iBailes As Integer
Dim iDorsales As Integer
Dim rs As Recordset
Dim sSQL As String
    ...
    ComprobarSiEstanTodasPuntuacionesDorsalesPresentes = C_FALTAN_PUNTUACIONES
    
    ' Comprobamos el número de dorsales presentes
    sSQL = " FROM dorsales WHERE cod_categoria = " & iCodCat & " AND repesca=" & iHojaRepesca & "AND fase = " & iFase
    Set rs = db.OpenRecordset("SELECT COUNT(*)" & sSQL & "  AND no_presente = 0 ", dbOpenSnapshot)
    iDorsales = rs.Fields(0)
    rs.Close
    ' Recuperamos los puestos introducidos de dorsales no presentes, por si no se recargó el PDA después de marcar un no presente
    Set rs = db.OpenRecordset("SELECT COUNT(*) FROM puntuaciones WHERE cod_categoria = " & iCodCat & " AND repesca=" & iHojaRepesca & " AND fase = " & iFase & " AND puntuaciones.num_dorsal IN (SELECT num_dorsal " & sSQL & "  AND no_presente > 0 " & ")", dbOpenSnapshot)
    iPuestosDorsalesNoPresentes = rs.Fields(0)
    rs.Close
    ' Comprobamos si hemos recopilado la información de todos los dorsales
    Set rs = db.OpenRecordset("SELECT COUNT(*) FROM puntuaciones WHERE cod_categoria = " & iCodCat & " AND repesca=" & iHojaRepesca & " AND fase = " & iFase, dbOpenSnapshot)
    iPuestos = rs.Fields(0)
    ...
    If G_CONTROL_PUNTUACIONES_NO_PRESENTES Then
        iPuestos = iPuestos - iPuestosDorsalesNoPresentes
    End If
    'Si estan todos OK, pero si hay cinco pasamos con C_PUNTUACIONES_CINCO_BAILES
    If iPuestos = iDorsales * iJueces * iBailes And iPuestos > 0 Then
        ComprobarSiEstanTodasPuntuacionesDorsalesPresentes = C_TODAS_PUNTUACIONES
    ElseIf iPuestos = iDorsales * iJueces * 5 And iPuestos > 0 Then
        ComprobarSiEstanTodasPuntuacionesDorsalesPresentes = C_PUNTUACIONES_CINCO_BAILES
    Else
        ComprobarSiEstanTodasPuntuacionesDorsalesPresentes = C_FALTAN_PUNTUACIONES
    End If

    
    If ComprobarSiEstanTodasPuntuacionesDorsalesPresentes > 0 Then
        'Si el número coincide, comprobamos que todas las puntuaciones corresponde con dorsales presentes
        sSQL = "SELECT COUNT(*) FROM dorsales d, puntuaciones p WHERE p.cod_categoria = " & iCodCat & " AND p.repesca=" & iHojaRepesca & " AND p.fase = " & iFase & _
                                  " AND p.cod_categoria=d.cod_categoria AND p.repesca=d.repesca AND p.fase=d.fase AND d.no_presente > 0 AND d.num_dorsal = p.num_dorsal"
        Debug.Print sSQL
        Set rs = db.OpenRecordset(sSQL, dbOpenSnapshot)
        iDorsales = rs.Fields(0)
        rs.Close
        If iDorsales > 0 Then
            MsgBox mml_FRASE1275, vbOKOnly Or vbCritical, "ERROR"
            ComprobarSiEstanTodasPuntuacionesDorsalesPresentes = C_FALTAN_PUNTUACIONES
        End If
    End If
...
Dim rs As Recordset
    ...
    rs.Close
End Function
Function ComprobarSiCountryAmateur(iCodCat As Integer) As Boolean
Dim rs As Recordset
    
    'Primero comprobamos si la hoja corresponde a un TeamMatch
    Set rs = db.OpenRecordset("SELECT COUNT(*) FROM categorias WHERE codigo = " & iCodCat & " AND (id_categoria = '" & C_COUNTRY_AMATEUR & "' OR cod_modalidad = " + G_COD_MODALIDAD_MEDALLAS + ")", dbOpenSnapshot)
    ComprobarSiCountryAmateur = IIf(rs.Fields(0) > 0, True, False)
    rs.Close
End Function
...
Sub CargarVariablesConfiguracion()
    ...
    G_CATEG_EQUIV_HORARIO = VarCfg("categ_equiv_horario", "A(""A"",""AI"",""AN""):B(""B"",""C"",""1T"",""2T""):D(""D"",""E""):F(""F"",""G"",""H"",""I"",""J"",""K"")", "Indica las categorías que consumen el mismo tiempo en pista. Cada grupo se separa por : y comienzan por la categ equivalente", "It indicates the categories that consume the same time in hint. Each group separates for : and they begin with the equivalent category")
    G_CATEG_XML_AEBDC = VarCfg("categ_xml_aebdc", """AI"",""AN"",""A"",""B"",""C"",""D"",""E"",""1T"",""2T""", "Categorías generadas en el fichero de intercambio XML para la AEBDC", "Categories generated in the exchange file XML for the AEBDC")
    G_COUNTRY_COMB_GRUP_HOJAS = IIf(VarCfg("country_combinar_grupos_en_hojas", "S", "Solo para Country, combina varios grupos en la misma hoja para cada baile", "Alone for Country, it combines several groups in the same leaf for each dance") = "S", True, False)
    G_BLOQUEAR_PAREJAS_AEBDC = IIf(VarCfg("aebdc_bloquear_busqueda_parejas", "S", "Bloquea la pantalla de parejas para permitir introducir solo parejas con código AEBDC", "This option Blocks the screen of couples to allow to introduce single couples with code AEBDC") = "S", 1, 0)
    G_IMP_POS_BAILES_DIPLOMAS = IIf(VarCfg("imprimir_posicion_bailes_diplomas", "S", "Imprime la posición final de cada baile en los diplomas", "IRIS Prints the final position of each dance in the diplomas") = "S", True, False)
    G_MARGEN_EPA_Y = VarCfg("cabecera_marge_y_imagenes", "300", "Altura a la que se posicionan las imágenes en las cabeceras de los documentos", "Height to which the images are printed in the heads of the documents")
    G_RESULTADOS_ALTO_CONTRASTE = IIf(VarCfg("publicacion_alto_contraste", "S", "Publicación de resultados en alto contraste", "Publication of results on high contrast") = "S", True, False)
    G_FORMATO_PAREJAS_PASOS = VarCfg("formato_import_parejas_pasos", "12, N", "Formato de importación de parejas de pasos (1:Primer apellido, 2:Segundo apellido, N:Nombre)", "Import format of Pasos couples")
    G_INTERVALO_CONTROL_CALCULO_PDA = Val(VarCfg("pda_intervalo_min_calculo", "60", "Intervalo mínimo entre cálculos automáticos de tablets y PDAs en segundos", "Minimum interval between automatic calculations of tablets and PDAs in seconds"))
    G_CONTROL_PUNTUACIONES_NO_PRESENTES = IIf(VarCfg("control_puntuaciones_no_presentes", "S", "Calcula automáticamente aunque las PDAs no hayan actualizado los no presentes", "It calculates automatically although the PDAs has not upgraded the information of the numbers not present") = "S", True, False)
    G_DIR_7ZIP = VarCfg("dir_7zip", "C:\Escrutinio", "Directorio de 7zip", "7zip directory")
    
    G_DIPLOMAS_PRIMER_DORSAL_SALON = VarCfg("diplomas_primer_dorsal_salon", "1", "Primer puesto de la final de salón en la que se imprime diploma", "First position of the final in which diploma is printed")
    G_COUNTRY_MODALID_SIN_GR_EDAD = VarCfg("country_mod_sin_grupo_edad", "|46|47|48|49|55|56|57|58|59|54|78|79|81|43|44|45|46|47|48|49|68|69|70|72|83|", "Modalidades sin división de grupos de edad", "Modalities without division of age groups")
    G_COUNTRY_MODALID_CALC_GRUPOS_EDAD = VarCfg("country_mod_calculo_grupos_edad", "2", "Modo de cálculos de los grupos de edad", "Way of calculations of the age groups")
    G_COUNTRY_IMP_TODOS_BAILES_JUNTOS = IIf(VarCfg("country_hojas_punto_todos_los_bailes", "S", "Imprime todos los bailes juntos en country", "Prints all dancing together in country") = "S", True, False)
    
    G_MIN_JUECES_DESCALIFICACION = Val(VarCfg("min_jueces_descalificacion", "2", "Número mínimo de jueces para aceptar una descalificación", "Minimum number of judges to accept a disqualification"))
    G_IMPRESORA_DEFECTO = VarCfg("impresora_por_defecto", "NA", "Impresora por defecto (NA para que pregunte)", "Default printer (NA not defined)")
    ...
    G_LIM_JUECES_PARA_TABLAS_FINAL_MA = Val(VarCfg("max_jueces_tablas_final_dobles_MA", "5", "Máximo número de jueces con los que se imprimirán tablas de baile en paralelo en la hoja de resultados de MatchAnalisys", "Maximum number of judges with those that dance charts will be printed in parallel in the leaf of results of MatchAnalisys"))
    C_FICHERO_FEDERACION_V2 = Val(VarCfg("version_fichero_parejas_febd", "2", "Versión de fichero de carga de códigos de parejas FEBD.1 -> 14 campos con número de socios, 2 -> 8 campos", "Versión de fichero de carga de códigos de parejas FEBD.1 -> 14 campos con número de socios, 2 -> 8 campos"))
    G_CATEGORIAS_EXPORT_PUNTUACIONES = VarCfg("categorias_exportar_puntuaciones", "'A','B','C','D','E','AI','AN','T1','T2','T3'", "Categorías que se exportarán en el fichero de puntuaciones de jueces", "Categories that will be exported in the judges' scores file")
    G_COD_MODALIDAD_MEDALLAS = VarCfg("country_cod_modalidad_medallas", "0", "Código de la modalidad de medallas (-1 desactivado)", "Medal mode code")
    G_EDADES_COUNTRY_SIN_CONTROL = VarCfg("country_edades_sin_control", "|Junior Teen|,|Gems|,|Teenage|,|Senior|,|Open Age|", "Edades sin control de fecha de nacimiento. Grabar entre || y separadas por comas", "Ages without control of date of birth. Record between || and separated by commas")
...
Function CodigoGrupoEdad(sNombre As String) As Integer
Dim rs As Recordset

    On Local Error GoTo error
    CodigoGrupoEdad = 0
    If sNombre <> "" Then
        Set rs = db.OpenRecordset("SELECT codigo FROM gruposedad WHERE nombre = '" & sNombre & "'", dbOpenSnapshot)
        If Not rs.EOF Then
            CodigoGrupoEdad = rs.Fields(0)
        End If
        rs.Close
    End If
    Exit Function
error:
    ProcesarError "CodigoGrupoEdad"
End Function

'*********************************************************************************************

Public Sub COPIA_DibujarTablaA(f As Printer, iPosX As Integer, iPosY As Integer, iFilas As Integer, iCols As Integer, aTabla() As String, aDefCelda() As TCelda, iAltoCelda As Integer, Optional iFiliIni As Integer = 0)
Dim iFila As Integer
Dim iCol As Integer
Dim sValor As String
Dim iSuper As Integer
...
Function DescripcionVersion() As String
    Select Case VERSION
        Case VERSION_VALENCIA:
            DescripcionVersion = "(Valencia)"
        Case VERSION_VIGO:
            DescripcionVersion = "(Vigo)"
        Case VERSION_COUNTRY_FEBD:
            DescripcionVersion = "(FEBD)"
        Case VERSION_COUNTRY_ASOCIACION:
            DescripcionVersion = "(Asociación Country)"
    End Select
End Function

Function VersionActiva(sFuncion As String) As Boolean
    VersionActiva = False
    
    Select Case sFuncion
        Case "IMPRESION_DORSALES_ELIMINADOS"
            Select Case VERSION
                Case VERSION_COUNTRY_FEBD:
                    VersionActiva = True
                Case VERSION_VALENCIA:
...
Sub ComprobarImpresoraPorDefecto()
Dim X As Printer
    If G_IMPRESORA_DEFECTO <> "NA" Then
        For Each X In Printers
            If X.DeviceName = G_IMPRESORA_DEFECTO Then
                Set Printer = X
                Exit Sub
            End If
        Next
        MsgBox mml_FRASE1297 + G_IMPRESORA_DEFECTO, vbOKOnly Or vbCritical, "ERROR"
    End If
End Sub
