...
Begin VB.Form frmMenu 
   ...
   Begin VB.Menu mnuArchivo 
      ...
      End
      Begin VB.Menu mnuJuezParametroTM 
         Caption         =   "mml_FRASE1289"
      End
      Begin VB.Menu mnuBarComp 
         Caption         =   "-"
      End
      Begin VB.Menu mnuImportarPasos 
      ...
      End
      Begin VB.Menu mml_ExportarPuntuaciones 
         Caption         =   "mml_FRASE1300"
      End
      Begin VB.Menu MnuBarImportar 
         Caption         =   "-"
      End
      ...
      End
      Begin VB.Menu mnuBarDorsales 
         Caption         =   "-"
      End
      Begin VB.Menu mnuCargarParejasAEBDC 
         Caption         =   "mml_FRASE1232"
      End
      Begin VB.Menu mnuGenExcelParticipantesPos 
         Caption         =   "mml_FRASE1263"
      End
   End
   Begin VB.Menu mnuCompeticion 
      ...
      Begin VB.Menu mnuBar3 
         Caption         =   "-"
      End
      Begin VB.Menu mnuMatchAnalysis 
         Caption         =   "MatchAnalysis"
      End
      Begin VB.Menu mnuMAPuntuaciones 
         Caption         =   "MA points"
      End
   ...
   Begin VB.Menu mnuArchivoAvanzado 
      ...
      End
      Begin VB.Menu mnuProDanceData 
         Caption         =   "mml_FRASE0842"
      End
      Begin VB.Menu mnuConfigHTML 
         Caption         =   "Config HTML"
      End
...
Sub ActualizacionBaseDatos()
    ...
    'Versión 2.21 con agrupaciones mejoradas y generación de fichero ZIP para publicación
    db.Execute "ALTER TABLE agrupaciones ADD descripcion varchar(100)"
    'Versión 2.20 mejoras para permitir TeamMatch con parámetros independiente para cada juez
    db.Execute "ALTER TABLE MA_parametros ALTER descorta varchar(10)"
    db.Execute "CREATE TABLE Juez_Parametro (cod_juez varchar(2), cod_categoria integer, cod_parametroTM integer, fase integer, posicion integer)"
    'Versión 2.10 mejoras para la importación de parejas de pasos
    db.Execute "ALTER TABLE CFG_DISP_HTML ADD IRISM_tamtablapuestos integer"
    If Err.Number = 0 Then
        db.Execute ("UPDATE CFG_DISP_HTML SET IRISM_tamtablapuestos = 600")
    End If
    Err.Clear
    db.Execute "ALTER TABLE CFG_DISP_HTML ADD IRISM_anchopuestosfinal integer"
    If Err.Number = 0 Then
        db.Execute ("UPDATE CFG_DISP_HTML SET IRISM_anchopuestosfinal = 90")
    End If
    Err.Clear
    db.Execute "ALTER TABLE CFG_DISP_HTML ADD IRISM_fuentepuestosfinal integer"
    If Err.Number = 0 Then
        db.Execute ("UPDATE CFG_DISP_HTML SET IRISM_fuentepuestosfinal = 60")
    End If
    Err.Clear
    db.Execute "ALTER TABLE cfg_html ADD pista varchar(5)"
    'Versión 2.9 mejoras para la importación de parejas de pasos
    db.Execute "ALTER TABLE DescCategoria ADD desc_importacion varchar(30)"
    'Versión 2.8 mejoras para generación dinámica de pantallas HTML de MA e IRISM
    db.Execute "CREATE TABLE CFG_DISP_HTML (id_juez varchar(2), IRISM_fuentefinal integer, IRISM_fuentesemi integer, IRISM_cortepuestosfinal integer, IRISM_puestoslineasemi integer, IRISM_fuentejuez integer, MA_fuente integer, MA_fuentevalor integer, MA_fuentebarra integer, MA_altobarra integer, MA_altocelda integer, MA_fuentejuez integer, MA_tamtabla integer)"
    If Err.Number = 0 Then
        db.Execute ("INSERT INTO CFG_DISP_HTML VALUES ('ZZ',100,100,6,6,35,55,70,30,70,110,30,1130)")
    End If
    Err.Clear
    'Versión 2.7 Mejoras para permitir MatchAnalisys
    db.Execute "ALTER TABLE MA_Parametros ADD activo varchar(1)"
    If Err.Number = 0 Then
        db.Execute ("INSERT INTO MA_Parametros VALUES (1,'TQ','Technical Quality', 10, 'S')")
        db.Execute ("INSERT INTO MA_Parametros VALUES (2,'MM','Movement to Music', 10, 'S')")
        db.Execute ("INSERT INTO MA_Parametros VALUES (3,'PS','Partnering Skill', 10, 'S')")
        db.Execute ("INSERT INTO MA_Parametros VALUES (4,'CP','Choreography and Presentation', 10, 'S')")
    End If
    Err.Clear
    db.Execute "CREATE TABLE MA_Parametros (codigo integer, descorta varchar(5), descripcion varchar(80), maxpuntuacion integer)"
    db.Execute "CREATE TABLE MA_puntuaciones (num_dorsal integer, cod_categoria integer, cod_baile integer, cod_juez varchar(2), fase integer, repesca integer, parametro integer, valor integer)"
    db.Execute "CREATE TABLE MatchAnalysis (cod_comp integer, cod_categoria integer, cod_baile integer, cod_fase varchar(2), cod_rep integer, fase varchar(15))"
    'Versión 2.6.10 Mejoras para permitir agregar divisiones de grupos en el horario
    db.Execute "CREATE TABLE FasesHorario (des_fase varchar(80), duracion integer, inicio_sesion integer)"
...
Private Sub cmdRecOptico_Click()
    ...
    frmEnlacePPC_HTML.Show vbNomodal, Me
End Sub
...
Private Sub mml_ExportarPuntuaciones_Click()
Dim rs As Recordset
Dim sSQL As String
...
Dim sFichero As String
    ...
    If VERSION = VERSION_VIGO Then
        'Genera un CSV con información de todas las puntuaciones de los jueces
        
        sSQL = "SELECT ju.codigo AS cod_juez, ju.nombre, pu.num_dorsal, pu.cod_juez AS letra_juez, pu.puesto, pu.fase, pu.repesca, ba.codigo AS cod_baile, ba.nombre, " & _
                "ba.abrev, ca.codigo AS cod_categoria, ca.descripcion, ca.id_categoria, pa.nombre_hombre, pa.num_socio_hombre, pa.nombre_mujer, pa.num_socio_mujer, pa.aebdc_codigo " & _
                "FROM puntuaciones AS pu, categorias AS ca, parejas AS pa, jueces AS ju, dorsales AS do, bailes AS ba, juez_categ AS jc " & _
                "WHERE do.cod_pareja = pa.codigo " & _
                "AND do.cod_categoria = ca.codigo " & _
                "AND do.cod_categoria = pu.cod_categoria AND do.fase = pu.fase AND do.repesca = pu.repesca AND do.num_dorsal = pu.num_dorsal " & _
                "AND pu.cod_baile = ba.codigo " & _
                "AND pu.cod_juez = jc.id_juez " & _
                "AND pu.cod_categoria = jc.cod_categoria " & _
                "AND ju.codigo = jc.cod_juez " & _
                "AND ca.id_categoria IN (" & G_CATEGORIAS_EXPORT_PUNTUACIONES & ") " & _
                "AND ca.cod_competicion =  " & CodCompActiva() & _
                " ORDER BY ca.descripcion, pu.fase, pu.repesca, ba.codigo, pu.cod_juez"
    
        Debug.Print sSQL
        Set rs = db.OpenRecordset(sSQL, dbOpenSnapshot)
        CD.FileName = "Puntuaciones.CSV"
        CD.ShowSave
        sFichero = CD.FileName
        iFile = FreeFile
        Open sFichero For Output As #iFile
        Print #iFile, "cod_juez;nombre_juez;num_dorsal;letra_juez;puesto_o_pase;fase;repesca;cod_baile;nombre_baile;abrev_baile;cod_categoria;descripcion;id_categoria;nombre_hombre;num_socio_hombre;nombre_mujer;num_socio_mujer;codigo_federacion"
        While Not rs.EOF
            Print #iFile, rs.Fields("cod_juez") & ";" & rs.Fields("ju.nombre") & ";" & rs.Fields("num_dorsal") & ";" & rs.Fields("letra_juez") & ";" & rs.Fields("puesto") & ";" & rs.Fields("fase") & ";" & _
                    rs.Fields("repesca") & ";" & rs.Fields("cod_baile") & ";" & rs.Fields("ba.nombre") & ";" & rs.Fields("abrev") & ";" & rs.Fields("cod_categoria") & ";" & rs.Fields("descripcion") & ";" & _
                    rs.Fields("id_categoria") & ";" & rs.Fields("nombre_hombre") & ";" & rs.Fields("num_socio_hombre") & ";" & rs.Fields("nombre_mujer") & ";" & rs.Fields("num_socio_mujer") & ";" & rs.Fields("aebdc_codigo") & ";"
            rs.MoveNext
        Wend
        rs.Close
        Close #iFile
    Else
        MsgBox "Funcionalidad no implementada", vbCritical Or vbOKOnly, "AVISO"
...
Private Sub mnuCargarParejasAEBDC_Click()
    ...
    If CD.FileName <> "" Then
        ...
        If C_FICHERO_FEDERACION_V2 = "2" Then
            While Not EOF(iFile)
            Dim ncampos As Integer
                ...
                sLinea = CambiarCadena("""", "`", sLinea)
                ncampos = DividirCampo(sLinea, aCampo(), Chr$(9))
                If Val(aCampo(0)) = 0 And ncampos > 3 Then
                    MsgBox "Linea " & iLinea & " - Error de lectura de fichero. El fichero debe contener los siguientes campos:cod_aebdc<tabulador>num_socio_el<tabulador>nombre_el<tabulador>apellido1_el<tabulador>apellido2_el<tabulador>num_socio_ella<tabulador>nombre_ella<tabulador>apellido1_ella<tabulador>apellido2_ella<enter>", vbOKOnly Or vbExclamation, "ERROR"
                    Exit Sub
                End If
                db.Execute "INSERT INTO parejas_aebdc VALUES ('" & aCampo(0) & "','" & aCampo(4) & "','" & aCampo(1) & " " & aCampo(2) & " " & aCampo(3) & "','" & aCampo(8) & "','" & aCampo(5) & " " & aCampo(6) & " " & aCampo(7) & "','" & aCampo(13) & "')"
            Wend
        Else
            While Not EOF(iFile)
                Inc iLinea
...
Private Sub mnuConfigHTML_Click()
    frmCfgHTML.Show vbModal
End Sub
...
Dim iCont As Integer
    'Comprobamos que hay menos de 6 forms de este tipo
    iCont = 0
    For i = 0 To Forms.Count - 1
        If Forms(i).Name = "frmEnlacePPC_HTML" Then
            iCont = iCont + 1
        End If
    Next
    
    If iCont < 6 Then
        Set frm = Forms.add("frmEnlacePPC_HTML")
        frm.Show vbNomodal
    Else
        MsgBox "Too many Forms", vbOKOnly, "ERRO"
    End If
End Sub
...
Private Sub mnuGenExcelParticipantesPos_Click()
Dim rs As Recordset
Dim sSQL As String
Dim iFile As Integer
Dim sFichero As String
    ...
    On Local Error GoTo error
    'Genera un CSV con información de todos los participantes de la competición activa y las posiciones en las que han quedado
    
    sSQL = "SELECT DISTINCT p.num_socio_hombre, p.num_socio_mujer, p.aebdc_codigo, p.nombre_hombre, p.nombre_mujer, c.descripcion, r.posicion FROM resumenfinales r, parejas p, categorias c, dorsales d " & _
                            " WHERE c.codigo = r.cod_categoria AND c.cod_competicion = " & CodCompActiva & " AND r.dorsal = d.num_dorsal AND d.cod_categoria = r.cod_categoria " & _
                            " AND d.cod_pareja = p.codigo AND p.cod_competicion = c.cod_competicion"
    Debug.Print sSQL
    Set rs = db.OpenRecordset(sSQL, dbOpenSnapshot)
    CD.FileName = "Parejas-Pos.CSV"
    CD.ShowSave
    sFichero = CD.FileName
    iFile = FreeFile
    Open sFichero For Output As #iFile
    Print #iFile, "num_socio_hombre;num_socio_mujer;febd_codigo;nombre_hombre;nombre_mujer;descripcion_grupo;posicion"
    While Not rs.EOF
        Print #iFile, rs.Fields("num_socio_hombre") & ";" & rs.Fields("num_socio_mujer") & ";" & rs.Fields("aebdc_codigo") & ";" & rs.Fields("nombre_hombre") & ";" & rs.Fields("nombre_mujer") & ";" & rs.Fields("descripcion") & ";" & rs.Fields("posicion")
        rs.MoveNext
    Wend
    rs.Close
    Close #iFile
...
Dim iMinEdad As Integer
    ...
    If G_COUNTRY Then
        ...
        'Modo similar a baile de salón
        If G_COUNTRY_MODALID_CALC_GRUPOS_EDAD = "2" Then
            If InStr("|" & Trim(Str(iMod)) & "|", G_COUNTRY_MODALID_SIN_GR_EDAD) Then
                CalcularGrupoEdad = "-"
            Else
                If iMaxEdad >= 0 And iMaxEdad <= 9 Then
                   CalcularGrupoEdad = "Junior I"
                ElseIf iMaxEdad >= 10 And iMaxEdad <= 13 Then
                    CalcularGrupoEdad = "Junior II"
                ElseIf iMaxEdad >= 14 And iMaxEdad <= 17 Then
                    CalcularGrupoEdad = "Junior III"
                ElseIf iMaxEdad >= 18 And iMaxEdad <= 24 Then
                    CalcularGrupoEdad = "Adult I"
                ElseIf iMaxEdad >= 25 And iMaxEdad <= 34 Then
                    CalcularGrupoEdad = "Adult II"
                ElseIf iMaxEdad >= 35 And iMaxEdad <= 44 Then
                    CalcularGrupoEdad = "Adult III"
                ElseIf iMaxEdad >= 45 And iMaxEdad <= 54 Then
                    CalcularGrupoEdad = "Senior I"
                ElseIf iMaxEdad >= 55 And iMaxEdad <= 64 Then
                    CalcularGrupoEdad = "Senior II"
                ElseIf iMaxEdad >= 65 And iMaxEdad <= 69 Then
                    CalcularGrupoEdad = "Gold"
                ElseIf iMaxEdad >= 70 Then
                    CalcularGrupoEdad = "Platinum"
                End If
            End If
        ' Nuevo sistema de grupos de edad para country (Edad a 1 de Setiembre)
        ElseIf G_COUNTRY_MODALID_CALC_GRUPOS_EDAD = "3" Then
            If InStr("|" & Trim(Str(iMod)) & "|", G_COUNTRY_MODALID_SIN_GR_EDAD) Then
                CalcularGrupoEdad = "-"
            Else
                If iMaxEdad >= 0 And iMaxEdad <= 4 Then
                   CalcularGrupoEdad = "Tiny Tots"
                ElseIf iMaxEdad >= 5 And iMaxEdad <= 6 Then
                    CalcularGrupoEdad = "Rugrats"
                ElseIf iMaxEdad >= 7 And iMaxEdad <= 9 Then
                    CalcularGrupoEdad = "Primary"
                ElseIf iMaxEdad >= 10 And iMaxEdad <= 12 Then
                    CalcularGrupoEdad = "Junior"
                ElseIf iMaxEdad >= 13 And iMaxEdad <= 15 Then
                    CalcularGrupoEdad = "Teen"
                ElseIf iMaxEdad >= 16 And iMaxEdad <= 24 Then
                    CalcularGrupoEdad = "Young Adult"
                ElseIf iMaxEdad >= 25 And iMaxEdad <= 45 Then
                    CalcularGrupoEdad = "Open"
                ElseIf iMaxEdad >= 46 And iMaxEdad <= 60 Then
                    CalcularGrupoEdad = "Silver"
                ElseIf iMaxEdad >= 61 And iMaxEdad <= 70 Then
                    CalcularGrupoEdad = "Gold"
                ElseIf iMaxEdad >= 71 Then
                    CalcularGrupoEdad = "Platinum"
                End If
            End If
        Else
...
Private Sub mnuImportarPasos_Click()
    frmImportarParejasPasos.Show vbModal
End Sub
...
Private Sub mnuJuezParametroTM_Click()
    frmAJuezParTM.Show vbModal
End Sub

Private Sub mnuMAPuntuaciones_Click()
    frmMAPuntuaciones.Show vbModal
End Sub

Private Sub mnuMatchAnalysis_Click()
    frmMatchAnalysis.Show
End Sub
...
Private Sub mnuProDanceData_Click()
    frmProDance.Show vbModal
End Sub
...
'    frmEnlacePPC_HTML1.Show vbNomodal, Me
...
Private Sub Form_Initialize()
    ...
    If C_DEBUG Then MsgBox "Modo depuración activado." & vbCrLf & IIf(C_ERROR_FECHA, "Control de fecha activado", ""), vbOKOnly Or vbInformation, "Aviso"
...
Dim sDirFichasCopia As String
    ...
    frmMenu.Caption = mml_FRASE0036 & " v" & App.Major & "." & App.Minor & " Build " & App.Revision & IIf(PROTECCION, mml_FRASE0156, mml_FRASE0157) & IIf(C_COUNTRY, "  (COUNTRY)", "") + DescripcionVersion()
        
    'Cargamos las variables de configuración *************************************************
    CargarCfg
    
    If G_IMPRESORA_DEFECTO <> "NA" Then
        MsgBox mml_FRASE1296 & " " & G_IMPRESORA_DEFECTO, vbOKOnly Or vbInformation, ""
...
Dim iCont As Integer
    ...
    iCont = 0
    For i = 0 To Forms.Count - 1
        If Forms(i).Name = "frmAPuntuacionesBaile" Then
            iCont = iCont + 1
        End If
    Next
    
    If iCont < 3 Then
        Set frm = Forms.add("frmAPuntuacionesBaile")
        frm.Show vbNomodal
    Else
        MsgBox "Too many Forms", vbOKOnly, "ERRO"
    End If
